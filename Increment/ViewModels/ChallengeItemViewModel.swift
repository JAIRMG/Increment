// ChallengeItemViewModel.swift, created on 12/07/21 by jair
...
struct ChallengeItemViewModel: Identifiable {
    ...
    var buttonCompleteText: String {
        isDayComplete ? "Completed" : "Mark Done"
    }
        ...
        case .toggleComplete:
            ...
            let activities = challenge.activities.map { activity -> Activity in
                if today == activity.date {
                    return Activity(date: today, isComplete: !activity.isComplete)
                } else {
                    return activity
                }
            }
